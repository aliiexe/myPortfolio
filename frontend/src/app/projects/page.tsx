...
export default function ProjectsPage() {
  ...
  const [heroProject] = getFeaturedProjects();
  const remainingProjects = allProjects.filter(
    (project) => !heroProject || project.slug !== heroProject.slug
  );

  const filters: { key: "all" | "cybersecurity" | "development" | "design" | "freelance"; label: string }[] = [
    { key: "all", label: "All" },
  ...
  const freelanceProjects = remainingProjects.filter(
      ...
      <Section>
        ...
        <div className="mb-10 max-w-3xl">
          ...
          <p className="mt-4 text-lg text-[var(--color-foreground-muted)]">
            Case studies and client work that balance security, engineering quality, and visual clarity.
          </p>
        </div>
        ...
        <div className="mb-12 flex flex-wrap items-center gap-3 text-xs">
          {filters.map((filter) => (
            <button
              ...
              type="button"
              className={cn(
                "cursor-default rounded-full border px-3 py-1.5 text-xs font-medium",
                filter.key === "all"
        ...
        {heroProject && (
          <section className="mb-20 overflow-hidden rounded-3xl border border-[var(--glass-border)] bg-[var(--glass-bg)] shadow-[var(--shadow-card)] backdrop-blur-[var(--glass-blur)]">
            <Link
              href={`/projects/${heroProject.slug}`}
              className="grid gap-8 no-underline lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)] lg:items-stretch"
            >
              <div className="relative overflow-hidden bg-[var(--color-bg-elevated)]">
                <Image
                ...
                <div className="pointer-events-none absolute inset-y-0 left-0 w-24 opacity-70" style={{ background: "linear-gradient(90deg, rgba(255,64,0,0.5), transparent)" }} />
              </div>
              <div className="flex flex-col justify-between border-t border-l border-[var(--glass-border)] bg-[var(--color-bg-surface)] p-8 lg:p-10">
                <header className="space-y-4">
                  <span className="inline-flex items-center gap-2 rounded-full bg-[var(--color-bg-elevated)] px-3 py-1 text-xs font-medium text-[var(--color-signature-muted)]">
                    ...
                    Featured · {categoryLabels[heroProject.category] ?? heroProject.category}
                  </span>
                  <h2
                    className="font-display text-2xl font-semibold text-[var(--color-foreground)] sm:text-3xl"
                    style={{ fontFamily: "var(--font-display)" }}
                  >
                    {heroProject.title}
                  </h2>
        ...
        {projectCategories.map((category, index) => {
          const byCategory = remainingProjects.filter(
            ...
            <section key={category} className="mb-20 space-y-8">
              <header className="flex flex-col gap-2 md:flex-row md:items-baseline md:justify-between">
                ...
                <p className="text-xs text-[var(--color-muted)]">
                  {category === "cybersecurity" && "Security-first and security-adjacent initiatives."}
                  {category === "development" && "Full-stack and web engineering work."}
                ...
                {byCategory.map((project, projectIndex) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={cn(
                          "relative overflow-hidden rounded-2xl bg-[var(--color-bg-elevated)]"
                        )}
                      >
                        <Image
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-sm text-[var(--color-foreground-muted)]">
                  Selected freelance projects across brands, businesses, and technical products.
                </p>
              </div>
              <Link
                href="/contact"
